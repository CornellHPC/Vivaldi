...
DATASETS = [
    {
        "bin_fname": "$PSCRATCH/poker.t.bin",
        "txt_fname": "$PSCRATCH/poker.t.txt",
        "zip_fname": "$PSCRATCH/poker.t.bz2",
    ...
    {
        "bin_fname": "$PSCRATCH/HIGGS.bin",
        "txt_fname": "$PSCRATCH/HIGGS.txt",
        "zip_fname": "$PSCRATCH/HIGGS.xz",
    ...
    {
        "bin_fname": "$PSCRATCH/mnist8m.scale.bin",
        "txt_fname": "$PSCRATCH/mnist8m.scale.txt",
        "zip_fname": "$PSCRATCH/mnist8m.scale.xz",
...
def request_p_prefix(p, nodes, log_dir, s_name):
    timestamp = str(timedelta(minutes=240))
...
def run_5_trials(
    ...
    main_args = f"{input_dataset_path} {m} {d} {k}"
    log_args = (
    ...
    f.write(
        f"  srun -N {nodes} --ntasks-per-node 4 --cpus-per-task 32 --cpu-bind cores -G {p} $EXE_PATH {main_args}\n"
    )
    f.write(f'  echo ""\n')
    f.write(f"done\n\n")
...
def download(dataset):
    url = dataset["url"]
    name = os.path.expandvars(dataset["name"])
    tar_file = os.path.expandvars(dataset["zip_fname"])
...
def extract(dataset):
    ...
    tar_file = os.path.expandvars(tar_file)
    txt_out = os.path.expandvars(txt_out)
...
def prepare(dataset):
    name = dataset["name"]
    txt_in = os.path.expandvars(dataset["txt_fname"])
    bin_out = os.path.expandvars(dataset["bin_fname"])
